...
import greeed from "./layout/greeed";
...
class App
{
	...
	init()
	{
		...
		if (urlPath == '') { // home
			...
			this.initGreeed();
			this.initLazyload();
		} else if (urlPath.match(/^archives$/g)) { // archives
			this._currentPage = 'archives';
			this.initTopLink();
			this.initTopLinkVisibilityManagement();
			this.initLazyload();
			this.initFilterSwitch();
		} else if (urlPath.match(/^projet\/[a-z0-9_\-]+$/g)) { // project
			this._currentPage = 'project';
			this.initTopLink();
			this.initTopLinkVisibilityManagement();
			this.initLazyload();
		} else if (urlPath.match(/^a\-propos$/g)) { // about
			this._currentPage = 'about';
			this.initTopLink();
			this.initTopLinkVisibilityManagement();
		} else if (urlPath.match(/^contact$/g)) { // contact
	...
	filterProjects()
	{
		...
		for ( ; i < len; i++) {
			...
			if ((this._currentFilterIds.date == 0 || currentItem.dataset.date == this._currentFilterIds.date) &&
